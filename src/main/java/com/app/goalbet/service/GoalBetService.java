...
public class GoalBetService {
	...
	public ResponseEntity<List<MatchPredictions>> getPredictions() {
		...
		for (int i = 0; i < matchDetails.getBody().getData().size(); i++) {
			try {
				String url = UtilityFunctions.addQueryParam(
						CommonConstants.DB_BASE_URL + CommonConstants.PREDICTION_STORE_ID, "matchId",
						BeanUtils.getProperty(matchDetails.getBody().getData().get(i), "_id"));
				ResponseEntity<ApiResult> predictions = collectionHelper.getAPIResponse(url,
						CommonConstants.PREDICTION_API_KEY);
	...
	public String insertPredictions(PredictionDetails predictionDetails) {
		String url = UtilityFunctions.addQueryParam(CommonConstants.DB_BASE_URL + CommonConstants.PREDICTION_STORE_ID,
				"matchId", predictionDetails.getMatchId());
		ResponseEntity<ApiResult> dupResult = collectionHelper
				.getAPIResponse(url + "&userId=" + predictionDetails.getUserId(), CommonConstants.PREDICTION_API_KEY);
		ApiResult res = dupResult.getBody();
		if (res.getData().size() == 0) {
			collectionHelper.insertPredictions(CommonConstants.DB_BASE_URL + CommonConstants.PREDICTION_STORE_ID,
					CommonConstants.PREDICTION_API_KEY, predictionDetails);
	...
	public ResponseEntity<ApiResult> getNextMatchDetails() {
		...
		String url = UtilityFunctions.addQueryParam(CommonConstants.DB_BASE_URL + CommonConstants.MATCH_STORE_ID,
				"Date", lt.toString());
	...
	public ResponseEntity<String> getUserDetails(String userId) {
		String url = UtilityFunctions.addQueryIdentifier(CommonConstants.DB_BASE_URL + CommonConstants.USER_STORE_ID,
				userId);
		...
		for (int i = 0; i < matchDetails.getBody().getData().size(); i++) {
			try {
				String url = UtilityFunctions.addQueryParam(
						CommonConstants.DB_BASE_URL + CommonConstants.PREDICTION_STORE_ID, "matchId",
						BeanUtils.getProperty(matchDetails.getBody().getData().get(i), "_id"));
				ResponseEntity<ApiResult> predictions = collectionHelper.getAPIResponse(url + "&userId=" + userId,
						CommonConstants.PREDICTION_API_KEY);
				...
				if (predictions.getBody().getData().size() == 0) {
					...
					LocalTime matchTime = LocalTime.parse(
							BeanUtils.getProperty(matchDetails.getBody().getData().get(i), "time").toString(), dtf);
