...
public class GoalBetService {
	...
	public ResponseEntity<String> getPredictions() {
		return collectionHelper.getAPI(CommonConstants.DB_BASE_URL + CommonConstants.PREDICTION_STORE_ID, CommonConstants.PREDICTION_API_KEY);
	}
	...
	public void insertPredictions(PredictionDetails predictionDetails) {
		...
		ResponseEntity<ApiResponse> r = collectionHelper.getAPIResponse(url + "&userId=" + predictionDetails.getUserId(), CommonConstants.PREDICTION_API_KEY);
		ApiResponse res = r.getBody();
	...
	public ResponseEntity<String> getNextMatchDetails() {
		...
		return collectionHelper.getAPI(url, CommonConstants.MATCH_API_KEY);
	}
